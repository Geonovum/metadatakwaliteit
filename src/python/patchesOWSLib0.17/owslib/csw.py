...
class CatalogueServiceWeb(object):
    ...
    def __init__(self, url, lang='en-US', version='2.0.2', timeout=10, skip_caps=False,
        ...
        # Thijs Brentjens: added cookies for Geonovum
        self.cookies = None
    ...
    def getrecordbyid(self, id=[], esn='full', outputschema=namespaces['csw'], format=outputformat, cookies=None):
        """
        ...
        - outputschema: the outputSchema (default is 'http://www.opengis.net/cat/csw/2.0.2')
        - format: the outputFormat (default is 'application/xml')
        - cookies: cookie header (added by Thijs Brentjens, for Geonovum)
        """
        ...
        # added by Thijs Brentjens, for Geonovum
    ...
    def _invoke(self):
        ...
        if isinstance(self.request, six.string_types):  # GET KVP
            ...
            self.response = openURL(self.request, None, 'Get', username=self.username, password=self.password, timeout=self.timeout, cookies=self.cookies).read()
        else:
            ...
            self.response = util.http_post(request_url, self.request, self.lang, self.timeout, self.username, self.password, self.cookies)
